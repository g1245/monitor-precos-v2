...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex justify-end mb-4 space-x-3">
            @auth
                <button id="saveProductBtn" data-product-id="{{ $product->id }}" class="flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors save-btn">
                ...
                <button id="priceAlertBtn" data-product-id="{{ $product->id }}" class="flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors alert-btn">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                    </svg>
                    <span class="alert-text">Criar Alerta</span>
                </button>
            @else
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function togglePriceAlert() {
                ...
                if (hasAlert) {
                    ...
                    // Create alert - show modal for target price (optional)
                    const targetPrice = prompt('Digite o preço alvo (opcional - deixe em branco para alertar sobre qualquer mudança):');
            ...
            function updateAlertButton(hasAlert) {
                const text = alertBtn.querySelector('.alert-text');
                
                if (hasAlert) {
                    alertBtn.classList.add('has-alert', 'bg-yellow-100', 'text-yellow-700');
                    alertBtn.classList.remove('text-gray-700', 'hover:text-primary');
                    text.textContent = 'Alerta Ativo';
                } else {
                    alertBtn.classList.remove('has-alert', 'bg-yellow-100', 'text-yellow-700');
                    alertBtn.classList.add('text-gray-700', 'hover:text-primary');
                    text.textContent = 'Criar Alerta';
                }
            }
